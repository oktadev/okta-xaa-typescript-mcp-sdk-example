...
- A **Behind the scenes** panel showing all four XAA steps live, with timings, decoded
  ID-JAG and access-token claims, and a button to re-run the flow via `CrossAppAccessProvider`
...
2. After sign-in, the flow runs automatically via `CrossAppAccessProvider`, which discovers the
   authorization server through RFC 9728 resource metadata (no hardcoded authorization server URL):
   the four steps light up in order in the **Behind the scenes** panel with real timings, and the
   checklist renders as soon as step 4 delivers the data.
